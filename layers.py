...
class GCMapInter(Layer):
    """Graph convolution layer."""
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
                 alpha=1., sparse_inputs=False, act=tf.nn.relu, bias=False,
                 featureless=False, **kwargs):
        super(GCMapInter, self).__init__(**kwargs)
        ...
        with tf.variable_scope(self.name + '_vars'):
            for i in range(len(self.support)):
                self.vars['weights_' + str(i)] = glorot([input_dim, output_dim],
                                                        name='weights_' + str(i))
                self.vars['weights_b_' + str(i)] = glorot(
    ...
    def _call(self, inputs):
        ...
        for i in range(len(self.support)):
            if not self.featureless:
                pre_sup = dot(x, self.vars['weights_' + str(i)],
                              sparse=self.sparse_inputs)
                pre_sup_b = dot(x, self.vars['weights_b_' + str(i)],
                                sparse=self.sparse_inputs)
            else:
                pre_sup = self.vars['weights_' + str(i)]
...
class GCMapInterAda(Layer):
    """Graph convolution layer."""
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
                 alpha=1., sparse_inputs=False, act=tf.nn.relu, bias=False,
                 featureless=False, **kwargs):
        super(GCMapInterAda, self).__init__(**kwargs)
    ...
    def _call(self, inputs):
        ...
        for i in range(len(self.support)):
            ...
            # calculate second order interaction: XW * sum(X)
            # pre_sup_sec_ord = tf.multiply(pre_sup, pre_sup_b) * self.alpha
            pre_sup_sec_ord = tf.multiply(pre_sup, pre_sup_b) * self.vars['alp']
            # pre_sup_all = tf.concat([pre_sup, pre_sup_sec_ord], axis=1)
            pre_sup_all = tf.add_n([pre_sup, pre_sup_b, pre_sup_sec_ord])
            support = dot(self.support[i], pre_sup_all, sparse=True)
            supports.append(support)
